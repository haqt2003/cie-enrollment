...
// Khối chương trình được tạo sẵn
const program = document.createRange().createContextualFragment(`
<div class="container__form-input">
    <div class="container__form-input-blockAll">
        <div class="container__form-input-block school" tabindex="0">
            <label for="school" class="container__form-input-block-label"><span>*</span>CÁC TRƯỜNG XÉT TUYỂN</label>
            <span class="container__form-input-block-label-main"></span>
        ...
        <div class="container__form-input-block major" tabindex="0">
    ...
    <div class="container__form-input-block-invisible"></div>
...
const major = document.createRange().createContextualFragment(`
...
// Xử lí thêm ngành học
function addMajors() {
    const addMajorBtns = $$('.container__form-input-block-add');
    inputItems = $$('.container__form-input-block');
    reconvertButton();
    addMajorBtns.forEach(function(addMajorBtn, index) {
        addMajorBtn.onclick = function() {
            const majors = addMajorBtn.parentElement.querySelector('.container__form-input-blockAll');
            majors.append(major.cloneNode(true));
            deleteProgram();
        }
    })
}

// Xử lí xóa chương trình/ngành học
function deleteProgram() {
    const delBtns = $$('.container__form-input-block-del');
    inputItems = $$('.container__form-input-block');
    reconvertButton();
    if(delBtns) {
        delBtns.forEach(function(delBtn) {
            delBtn.onclick = function() {
                const block = delBtn.closest('.container__form-input-block');
                if(block.matches('.school')) {
                    block.parentElement.parentElement.remove();
                }
                else if(block.matches('.major')) {
